data "terraform_remote_state" "dns" {
  ...
  config = {
    ...
    key     = "dns/${var.environment}/terraform.tfstate"
    region  = "us-west-2"
    encrypt = true
...
locals {
  ...
  lambda_zone_id           = var.lambda_zone_id_source == "public_zone_id" ? data.terraform_remote_state.dns.outputs.public_zone_id : (try(data.terraform_remote_state.dns.outputs.zones.lambda.public_zone_id, data.terraform_remote_state.dns.outputs.public_zone_id))
