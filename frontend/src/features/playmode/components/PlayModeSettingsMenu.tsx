...
export function PlayModeSettingsMenu({
	...
	const nextLayout =
		settings.layoutMode === 'single' ? 'columns-song' : settings.layoutMode === 'columns-song' ? 'columns-sections' : 'single';
	const layoutLabel =
		settings.layoutMode === 'single'
			? 'Layout: Mobile'
			: settings.layoutMode === 'columns-song'
				? 'Layout: 2 Columns'
				: 'Layout: Section Split';
	const layoutColor = settings.layoutMode === 'single' ? 'gray' : settings.layoutMode === 'columns-song' ? 'blue' : 'teal';
	const LayoutIcon =
		settings.layoutMode === 'single' ? IconList : settings.layoutMode === 'columns-song' ? IconColumns2 : IconColumns3;
	...
	return (
		<Menu shadow="md" width={300} position="bottom-start" closeOnItemClick={false}>
			...
			<Menu.Dropdown>
				...
				{isLayoutSwitchEnabled && (
					<Menu.Item
						leftSection={<LayoutIcon size={14} />}
						color={layoutColor}
